...
<table class="table table-bordered table-hover">
    ...
    <th style="text-align: center;">问卷描述</th>
    ...
    <#if recharge?? && recharge.list?? && recharge.list?size gt 0>
        ...
        <tr style="text-align: center;">
            ...
            <#if recharge.releaseState == '0'>
               <a href="javascript:Duang.getService('ydQuestionnaireService').edit('${recharge.id}')" class="btn btn-link" role="button" ><i class="fa fa-pencil-square"></i> 编辑</a>
                ...
                <a onclick="Duang.getService('ydQuestionnaireService').delete('${recharge.id}')" class="btn btn-link" ><i class="fa fa-trash-o"></i> 删除</a>
                <a onclick="Duang.getService('ydQuestionnaireService').manager('${recharge.id}')" class="btn btn-link" ><i class="fa fa-trash-o"></i> 查看问题</a>
        ...
        <#if recharge.pages gt 0>
            $("#pagenation").jqPaginator({
                ...
                onPageChange: function (num, type) {
                    if(type!='init'){
                        Duang.getService('ydQuestionnaireService').pageList(num);
                    }
                }
            })
        </#if>
