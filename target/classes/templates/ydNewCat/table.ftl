...
<table class="table table-bordered table-hover">
    <thead>
    <th style="text-align: center;">新闻分类名称</th>
    ...
    <#if recharge?? && recharge.list?? && recharge.list?size gt 0>
        ...
        <tr style="text-align: center;">
            ...
            <td class="text-center">
                <a href="javascript:Duang.getService('ydNewCatService').edit('${recharge.id}')" class="btn btn-link" role="button" ><i class="fa fa-pencil-square"></i> 编辑</a>
                <a onclick="Duang.getService('ydNewCatService').delete('${recharge.id}')" class="btn btn-link" ><i class="fa fa-trash-o"></i> 删除</a>
            </td>
        </tr>
        </#list>
    </#if>
        ...
        <#if recharge.pages gt 0>
            $("#pagenation").jqPaginator({
                ...
                onPageChange: function (num, type) {
                    if(type!='init'){
                        Duang.getService('ydNewCatService').pageList(num);
                    }
                }
            })
        </#if>
