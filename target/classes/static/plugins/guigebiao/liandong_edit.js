	...
	var Creat_Table = function(){
		...
		if (bCheck == true) {
			...
			$.each(arrayTile, function (index, item) {
				 item=item.replace("<input type=\"button\" value=\"删除\" onclick=\"deleteSkuEdit('Father_Item"+index+"')\">","");
                item=item.replace("<input type=\"button\" value=\"删除\" onclick=\"deleteSkuEdit('&quot;Father_Item"+index+"&quot;')\">","");
			...
			//未全选中,清除表格
			document.getElementById('createTableEdit').innerHTML="";
		}
	};
    var step = {
		//合并行
...
/*		$('#productItemjson').val(vsjonArray);
		...
		$('#productItemjson_update').val(vsjonArray)
        $('#productItemjsonhz_update').val(vsjonhzArray);
		//到这里已经封装完毕
