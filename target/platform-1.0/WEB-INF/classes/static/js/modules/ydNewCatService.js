var ydNewCatService = new Class({
    Extends: project.Service,
    options: {
        serviceName: 'ydNewCatService',
    ...
    loadTable: function () {
        ...
        $.get(project.path + '/ydNewCat/table',param,function(data){
    ...
    reset:function(){
        ...
        $.get(project.path + '/ydNewCat/table',function(data){
            $('#'+_this.options.contentName).html(data);
        })
    },
    edit: function (id) {
        $("#" + this.options.modalName).modal({
            remote: project.path + "/ydNewCat/edit?id=" + id
        });
    },
    add: function () {
        $("#" + this.options.modalName).modal({
            remote: project.path + "/ydNewCat/insert"
        });
    },
    delete: function (id) {
        ...
        if (r) {
            var $this = this;
            $.post(project.path + '/ydNewCat/delete?id=' + id, function (data) {
                if (data.success) {
                    $this.loadTable();
    ...
    saveOrUpdate: function () {
        ...
        var param = $('#' + this.options.formName).formToJson();
                ...
                $.post(project.path + '/ydNewCat/save', param, function (data) {
                    if (data.success) {
