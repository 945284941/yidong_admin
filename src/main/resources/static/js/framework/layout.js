...
var Layout = function () {
    ...
    return {
        ...
        loadAjaxContent: function (url, sidebarMenuLink) {
            ...
            var ajaxPageContent = $('.page-content .page-content-body');
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    if (sidebarMenuLink && sidebarMenuLink.size() > 0 && sidebarMenuLink.parents('li.open').size() === 0) {
                        $('.page-sidebar-menu > li.open > a').click();
                    }

                    ajaxPageContent.html(res);
                ...
                error: function (res, ajaxOptions, thrownError) {
                    //App.stopPageLoading();
                    Duang.removeMask();
                    ajaxPageContent.load(project.path + '/errorPage');
        ...
        initGlobalAjaxEvent: function () {
            $.ajaxSetup({
                ...
                statusCode: {
                    404: function () {
                        Duang.error("页面丢失...opps!!");
                        return;
                    },
                    500: function () {
                        Duang.error("系统内部错误!请联系管理员!");
                        return;
                    }
                },
                error: function (jqXHR, textStatus, errorThrown) {
                    Duang.removeMask();
                    if (jqXHR.status == 500) {
                        var error = jqXHR.getResponseHeader('session-status');
                        if (error) {
                            Duang.error(jqXHR.getResponseHeader('session-message'));
                        }
                    }
                }
            });
            $(document).ajaxComplete(function (event, xhr, settings) {
                ...
                var error = xhr.getResponseHeader('session-status');
                if (error) {
                    var r = window.confirm("由于您长时间未操作,请重新登录!");
                    if (r) {
                        top.location.href = "/";
                    }
                    return;
