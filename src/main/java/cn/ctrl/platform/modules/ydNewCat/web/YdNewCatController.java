package cn.ctrl.platform.modules.ydNewCat.web;
 ...
 */
@Controller
@RequestMapping("ydNewCat")
public class YdNewCatController extends BaseController {

    @Autowired
    YdNewCatService rechargeService;


    @GetMapping("manager")
    public String manager(Model model){
        return "ydNewCat/manager";
    }
    ...
    public String table(Model model,@RequestParam Map map,@RequestParam(value = "pageNo", defaultValue = "1") int pageNo,
        ...
        return "ydNewCat/table";
    }


    @GetMapping("insert")
    public String add(Model model){
        YdNewCat classification =new YdNewCat();
        model.addAttribute("recharge",classification);
        return "ydNewCat/form";
    }
    @PostMapping("save")
    @ResponseBody
    public JsonContent save(@RequestParam Map  map, HttpServletRequest request, YdNewCat recharge){
 ...
 public JsonContent imgSave(@RequestParam("file") MultipartFile file)  {
    ...
    public String edit(Model model,String id){
        ...
        YdNewCat recharge = rechargeService.selectOne(id);
