package cn.ctrl.platform.modules.aliyun;
...
public class AliyunOSSClientUtil {
   ...
   /* private static  OSSClient ossClient=AliyunOSSClientUtil.getOSSClient();*/
    ...
    public static void main(String[] args) {
        ...
        //上传文件
        String files="H:\\Jason\\pic\\vvv.jpg";
        String[] file=files.split(",");
        for(String filename:file){
            //System.out.println("filename:"+filename);
            File filess=new File(filename);
            String md5key = AliyunOSSClientUtil.uploadObject2OSS(ossClient, filess, BACKET_NAME, FOLDER).get("resultStr").toString();
     ...
     * @return Map 返回的唯一MD5数字签名 以及 对应的上传到文件的key
     * */
    public static Map<String,Object> uploadObject2OSSnew(OSSClient ossClient, MultipartFile f, String bucketName, String folder) {
        String resultStr = null;
        Map map=new HashMap();
        try {
            //以输入流的形式上传文件
    ...
    public static String getFileUrl(File file){
       ...
       OSSClient ossClient= AliyunOSSClientUtil.getOSSClient();
       Map map=uploadObject2OSS(ossClient,file,BACKET_NAME,"/pic");
       return getUrl(map.get("imageUrl").toString());
    }

    public static String getFileUrlNew(MultipartFile m){
        OSSClient ossClient= AliyunOSSClientUtil.getOSSClient();
        Map map=uploadObject2OSSnew(ossClient,m,BACKET_NAME,"/pic");
        return getUrl(map.get("imageUrl").toString());
    }
