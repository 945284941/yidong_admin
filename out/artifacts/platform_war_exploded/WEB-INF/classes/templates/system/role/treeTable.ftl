...
<tr data-tt-id="${root.id}" data-tt-parent-id="${pid}"
    <#if firstRoot==1>style="display: none;" </#if>class="branch collapsed">
    <td><span style="padding-left: 0px;"></span>${(root.name)!"角色名称丢失"}</td>
    <td>${(root.remark)!""}</td>
    <td class="text-center">
        <#if firstRoot==1>
            <@shiro.hasPermission name="role_manager_edit">
                <a href="javascript:Duang.getService('roleservice').edit('${root.id}')" class="btn btn-link"
                   role="button"><i class="fa fa-pencil-square"></i>编辑&授权</a>
            </@shiro.hasPermission>

            <@shiro.hasPermission name="role_manager_delete">
                <a href="javascript:Duang.getService('roleservice').delete('${root.id}')" class="btn btn-link"><i
